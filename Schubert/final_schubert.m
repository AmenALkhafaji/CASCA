...
X1=table2array(Datos(:, 1:(size(Datos,2)-3)));
depth = sum(X, 2);
...
% Normalize each oMEDA vector so that sum of squared values = 1
rjList = {rj1, rj2, rj3, rj4};

for k = 1:numel(rjList)
normVal = sqrt(sum(rjList{k}(:).^2));

if normVal > 0
    rjList{k} = rjList{k} ./ normVal;
end
...
% NOTE: Panel E stars were intentionally removed.
%       Panel E now shows only signed oMEDA direction by colour
%       and absolute oMEDA magnitude as cell text.
%% ============================================================

function plotPanelOMEDAHeatmap_NoTaxa(axMain, axCB, OMEDA_top, Sig_top, methodNames) %#ok<INUSD>
    ...
    % Cell colours remain signed. Printed values are absolute magnitudes.
    addHeatmapNumbersAbs(axMain, OMEDA_top, 2, 0.55 * maxAbs, 6.3);

    % No significance stars are drawn in Panel E.
    ...
    cb.TickLabels = {sprintf('%.2f', -maxAbs), '0', sprintf('%.2f', maxAbs)};
