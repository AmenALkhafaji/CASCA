%% ============================================================
% SCHUBERT PANEL DATA:
% C-ASCA SCORES + C-ASCA LOADINGS WITH parglmMC STARS
% + METHOD-WISE EFFECT HEATMAP
%
...
%           star   = parglmMC p <= alpha
%
% Panel E = Method-wise Median(Case) - Median(Control)
%           colour = signed displayed effect
%           number = absolute displayed effect
%           star   = KW/BH-FDR significance
%
% IMPORTANT:
%   - Do NOT normalize X_RAW/X_TSS/X_RARE/X_CLR before KW/effect calculation.
%   - Panel E display normalization is applied only after selecting top taxa.
%   - Each Panel E column is normalized separately across the selected taxa.
%   - Stars are kept.
%   - Cell numbers show absolute values.
%   - Signed colours are preserved.
%% ============================================================
...
%   X        : multiblock matrix used for C-ASCA / parglmMC
%   X1       : method-specific matrix used for method-wise Panel E effects
%   Method
%   Group
%   lab
%   ascao
...
if ~exist('X1', 'var')
    error('Variable X1 is required in workspace for method-wise Panel E effects.');
end
...
if size(X1,2) ~= nTaxa
    error('Number of columns in X1 must match number of taxa labels in lab.');
end
...
if size(X1,1) ~= size(X,1)
    error('X1 and X must have the same number of rows.');
end
...
%
% FINAL DECISION:
% Do NOT unit-normalize these blocks before calculating Panel E.
% Panel E effects and KW/BH-FDR are calculated from X1 method blocks
% on their original analysis scale.
%% ============================================================
...
%% ============================================================
% FEATURE-WISE KRUSKAL-WALLIS + BH-FDR FOR PANEL E
%
% Effect direction:
%   positive = higher in Case/CDI
%   negative = higher in Control/Healthy
%% ============================================================
...
% C-ASCA LOADINGS
%
% Consensus sign-orientation rule is kept.
%% ============================================================
...
%% ============================================================
% parglmMC TAXON-LEVEL SIGNIFICANCE FOR PANEL D
%
% Factor 1 = Method / preprocessing disagreement
% Factor 2 = Group / biological consensus
%
% p-values are used only as stars over the C-ASCA loading heatmap.
%% ============================================================

if exist('observation', 'var') && exist('depth', 'var')

    fprintf('\nUsing full parglmMC design: Method + Group + observation + depth.\n');

    F_mc = [Method(:), Group(:), observation(:), depth(:)];

    [T_mc, parglmMCStruct] = parglmMC(X, F_mc, ...
        'Preprocessing', 1, ...
        'Model', [1 2], ...
        'Ordinal', [0 0 0 1], ...
        'Random',  [0 0 1 0], ...
    ...
    warning(['observation and/or depth not found. ', ...
             'Using simplified two-factor parglmMC design: Method + Group. ', ...
...
p_disagreement = parglmMCStruct.p(:,1);   % Method / preprocessing
p_consensus    = parglmMCStruct.p(:,2);   % Group / biological consensus
...
%% ============================================================
% PANEL E DISPLAY MATRIX
%
% Effect_top_raw keeps the real median Case-Control effects.
% Effect_top is normalized only for display in Panel E.
%
% Each Panel E column is normalized separately across the selected taxa
% to unit Euclidean norm:
%
%     sum(Effect_top(:,c).^2) = 1
%
% This does NOT force the largest value in each column to be 1.
% Therefore, individual cells may remain below 1.
%
% This does NOT affect p-values or significance stars.
%% ============================================================

Effect_top_raw = Effect_all_raw(idxTop, :);
Sig_top        = Sig_all(idxTop, :);

Effect_top = Effect_top_raw;

% Unit-norm display normalization for each Panel E column.
% Columns:
% 1 = Raw Data
% 2 = TSS
% 3 = Rarefaction
...
for c = colsToScale
    ...
    if nrm < eps || isnan(nrm)
        warning('Panel E column %d has near-zero norm. Returning zeros for display.', c);
        Effect_top(:, c) = zeros(size(col));
    else
        Effect_top(:, c) = col ./ nrm;
    end

end

% Diagnostic check: each displayed column should have sum of squares = 1
fprintf('\nPanel E display-normalization check\n');
for c = colsToScale
    fprintf('%s: sumsq = %.6f | norm = %.6f | maxAbs = %.6f\n', ...
        methodNamesEffect(c), ...
        sum(Effect_top(:,c).^2, 'omitnan'), ...
        norm(Effect_top(:,c), 2), ...
        max(abs(Effect_top(:,c)), [], 'omitnan'));
end
...
Results_table.AbsConsensusLoading    = abs(load_consensus(:));
Results_table.AbsDisagreementLoading = abs(load_disagree(:));
...
disp(Results_table(1:min(25,height(Results_table)),:));
...
plotTwoColumnLoadingHeatmapWithStars( ...
    L_top, ...
    P_top, ...
    SigLoad_top, ...
    taxa_top, ...
    loadingNames, ...
    fullfile(outDir, "Schubert_ASCA_loadings_with_parglmMC_stars"), ...
    exportResolution);
...
plotMethodEffectHeatmapCompact( ...
    ...
    methodNamesEffect, ...
    Sig_top, ...
    fullfile(outDir, "Schubert_methodwise_effect_heatmap_column_scaled"), ...
    exportResolution);
...
fprintf('\nSchubert C-ASCA panel saved in:\n%s\n\n', outDir);

%% ============================================================
% INDIVIDUAL LOADING HEATMAP WITH STARS
%% ============================================================

function plotTwoColumnLoadingHeatmapWithStars(L_plot, Pvals, SigMask, taxaNames, colNames, outBase, exportResolution)
    ...
    addHeatmapNumbers(axLoad, L_plot, 2, 0.55, 7.2);
...
function plotMethodEffectHeatmapCompact(Effect_top, taxaNames, methodNames, Sig_top, outBase, exportResolution)
    ...
    addStarMarkers(axEff, Sig_top, 8.0, -0.34);
...
function createFinal_CASCA_MethodEffect_Panel( ...
    ...
    Effect_top, Sig_top, effectMethodNames, outBase, analysisLevel)
    ...
    plotPanelLoadingHeatmapWithStars( ...
        axDmain, ...
        axDtaxa, ...
        axDcb, ...
        L_top, ...
...
%% ============================================================
% PANEL D: LOADING HEATMAP WITH STARS
%% ============================================================

function plotPanelLoadingHeatmapWithStars(axMain, axTaxa, axCB, L_plot, Pvals, SigMask, taxaNames, colNames)
    ...
    addHeatmapNumbers(axMain, L_plot, 2, 0.55, 6.8);
...
function plotPanelMethodEffectHeatmap_NoTaxa(axMain, axCB, Effect_top, Sig_top, methodNames)
    ...
    addStarMarkers(axMain, Sig_top, 7.2, 0.22);
...
% Keeps 0 instead of -0.00 or 0.00.
%% ============================================================
