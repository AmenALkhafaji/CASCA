...
%
%  Final panel:
%    A = Factor 1 / method-related score plot (9 methods)
...
%        no significance stars are displayed
%
%  Important:
...
close all;
clc;
clear figLoad figOmeda figComb
close all force
drawnow
clear all;
...
analysisLevel = "phylum";   % "genus" or "phylum"
...
switch lower(analysisLevel)
    ...
    case "genus"
        ...
        dataFile = load('GenusASCA1.mat');
        ascaFile = load('GenusASCA1.mat'); %#ok<NASGU>

        if ~isfield(dataFile, 'Datos')
            error('genus.mat does not contain a variable named Datos.');
        end
        ...
        outDir = fullfile( ...
            pwd, ...
            'Final_Genus_CASCA_oMEDA_Figures' ...
        );
    ...
    case "phylum"
        ...
        dataFile = load('PhylumASCA1.mat');
        ascaFile = load('PhylumASCA1.mat'); %#ok<NASGU>

        if ~isfield(dataFile, 'Datos')
            error('phylum.mat does not contain a variable named Datos.');
        end
        ...
        outDir = fullfile( ...
            pwd, ...
            'Final_Phylum_CASCA_oMEDA_Figures' ...
        );
...
% Validate the expected nine-method structure.
expectedMethods = 10;
expectedRowsPerMethod = 300;
expectedTotalRows = expectedMethods * expectedRowsPerMethod;

fprintf( ...
    'Loaded Datos dimensions: %d rows x %d columns\n', ...
    height(Datos), ...
    width(Datos) ...
);
...
if height(Datos) ~= expectedTotalRows
    error( ...
        ['Datos contains %d rows, but nine methods with 300 observations ' ...
         'require %d rows. Ensure that phylum.mat or genus.mat contains ' ...
         'the combined nine-method table.'], ...
        height(Datos), ...
        expectedTotalRows ...
    );
end
...
% Methods, Groups, observation
X = table2array(Datos(:, 1:(size(Datos,2)-3)));

depth = sum(X, 2);

% Method codes must be numeric values 1:9.
...
if any(~ismember(MethodCode, 1:9))
    invalidCodes = unique(MethodCode(~ismember(MethodCode, 1:9)));
    error( ...
...
% Convert numeric method codes to plotting labels.
Method = strings(size(MethodCode));

Method(MethodCode == 1) = "Raw Data";
Method(MethodCode == 2) = "TSS";
Method(MethodCode == 3) = "Rarefaction";
Method(MethodCode == 4) = "CLR";
Method(MethodCode == 5) = "CSS";
Method(MethodCode == 6) = "edgeR-TMM";
Method(MethodCode == 7) = "DESeq2";
Method(MethodCode == 8) = "ALDEx2";
Method(MethodCode == 9) = "ANCOM";
...
if isnumeric(Groups)
    ...
    if any(~ismember(Groups, [1 2]))
        error('Numeric Datos.Groups must contain only 1 and 2.');
    end
...
% Validate row order and within-method observation numbering.
expectedMethodSequence = repelem((1:9)', expectedRowsPerMethod);

if ~isequal(MethodCode(:), expectedMethodSequence)
    error( ...
        ['Rows must be ordered by method as 300 rows for code 1, then 300 ' ...
         'rows for code 2, continuing through code 9.'] ...
    );
end

expectedObservationSequence = repmat((1:expectedRowsPerMethod)', expectedMethods, 1);

if ~isequal(observation(:), expectedObservationSequence)
    error( ...
        ['Datos.observation must run from 1 to 300 within each method block.'] ...
    );
end
...
if mod(size(X,1), num_parts) ~= 0
    error( ...
        'Number of rows in X (%d) must be divisible by 9.', ...
        size(X,1) ...
    );
end
...
methodOrder = ["Raw Data","TSS","Rarefaction","CLR","CSS","edgeR-TMM","DESeq2","ALDEx2","ANCOM"];
...
load_disagree = ascao.factors{1}.loads(:,1) ;
...
rowsPerMethod = size(X,1) / 9;

if rowsPerMethod ~= 300
    warning( ...
        'Expected 300 rows per method, but detected %.0f rows per method.', ...
        rowsPerMethod ...
    );
end

groupReference = Group(1:rowsPerMethod);
X=X2;
rj1 = omedaPca(X(1*rowsPerMethod-rowsPerMethod+1 : 1*rowsPerMethod,:), ...
    [1], X(1*rowsPerMethod-rowsPerMethod+1 : 1*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj2 = omedaPca(X(2*rowsPerMethod-rowsPerMethod+1 : 2*rowsPerMethod,:), ...
    [1], X(2*rowsPerMethod-rowsPerMethod+1 : 2*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj3 = omedaPca(X(3*rowsPerMethod-rowsPerMethod+1 : 3*rowsPerMethod,:), ...
    [1], X(3*rowsPerMethod-rowsPerMethod+1 : 3*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj4 = omedaPca(X(4*rowsPerMethod-rowsPerMethod+1 : 4*rowsPerMethod,:), ...
    [1], X(4*rowsPerMethod-rowsPerMethod+1 : 4*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj5 = omedaPca(X(5*rowsPerMethod-rowsPerMethod+1 : 5*rowsPerMethod,:), ...
    [1], X(5*rowsPerMethod-rowsPerMethod+1 : 5*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj6 = omedaPca(X(6*rowsPerMethod-rowsPerMethod+1 : 6*rowsPerMethod,:), ...
    [1], X(6*rowsPerMethod-rowsPerMethod+1 : 6*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj7 = omedaPca(X(7*rowsPerMethod-rowsPerMethod+1 : 7*rowsPerMethod,:), ...
    [1], X(7*rowsPerMethod-rowsPerMethod+1 : 7*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj8 = omedaPca(X(8*rowsPerMethod-rowsPerMethod+1 : 8*rowsPerMethod,:), ...
    [1], X(8*rowsPerMethod-rowsPerMethod+1 : 8*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);

rj9 = omedaPca(X(9*rowsPerMethod-rowsPerMethod+1 : 9*rowsPerMethod,:), ...
    [1], X(9*rowsPerMethod-rowsPerMethod+1 : 9*rowsPerMethod,:), ...
    groupReference, 'Preprocessing', 1);
...
if size(X_consensus_repeated, 1) ~= ...
        rowsPerMethod * numberOfMethods

    error( ...
        ['Expected X_consensus_repeated to contain %d rows, ' ...
...
% Average corresponding observations across the nine methods.
X_consensus = mean( ...
    X_consensus_3D, ...
    2, ...
    'omitnan' ...
...
% Convert from 300 × 1 × taxa to 300 × taxa.
X_consensus = reshape( ...
    X_consensus, ...
    rowsPerMethod, ...
    nVariables ...
...
% The group vector must also contain only 300 observations.
Group_consensus = Group(1:rowsPerMethod);
...
rj6  = normalizeVector(rj6);
rj7  = normalizeVector(rj7);
rj8  = normalizeVector(rj8);
rj9  = normalizeVector(rj9);
rj10 = normalizeVector(rj10);
...
%
% Stars in Panel E use the group-related parglmMC p-values.
% The same group-effect significance is repeated across all ten oMEDA
% columns. Stars are suppressed when the displayed oMEDA value rounds to 0.
%% ============================================================

nDecPanelE = 2;
zeroTolPanelE = 0.5 * 10^(-nDecPanelE);

Sig_OMEDA_raw = repmat(p_consensus(:) <= alpha, 1, 10);
Sig_OMEDA = Sig_OMEDA_raw & abs(O) >= zeroTolPanelE;

plotOmedaHeatmapCompact( ...
    O, ...
    Sig_OMEDA, ...
    ["Raw Data","TSS","Rarefaction","CLR","CSS","edgeR-TMM","DESeq2","ALDEx2","ANCOM","C-ASCA"], ...
