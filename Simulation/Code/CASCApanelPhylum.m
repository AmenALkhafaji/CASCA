...
%
%  Final panel:
%    A = Factor 1 / method-related score plot (NO legend)
%    B = Factor 2 / group-related score plot (NO legend)
%    C = Combined model score plot (WITH legend)
%    D = C-ASCA loading heatmap WITH taxa labels
%    E = oMEDA over PCA heatmap WITHOUT taxa labels
%
%  Layout updates:
%    - Fixed width = 7.2 inches
%    - Increased height = 7.8 inches
%    - A/B/C kept compact
%    - D/E made taller
%    - E has no taxa-label axis
%    - Higher-resolution PNG export
%    - Vector PDF export retained
%
%  Export:
...
if numel(taxaNames) ~= numel(load_disagree) || numel(taxaNames) ~= numel(load_consensus)
    error('taxaNames, load_disagree, and load_consensus must have the same length.');
end
...
rj1 = omedaPca(X(1:300,:),    [1], X(1:300,:),    Group(1:300), 'Preprocessing', 1);
rj2 = omedaPca(X(301:600,:),  [1], X(301:600,:),  Group(1:300), 'Preprocessing', 1);
rj3 = omedaPca(X(601:900,:),  [1], X(601:900,:),  Group(1:300), 'Preprocessing', 1);
rj4 = omedaPca(X(901:1200,:), [1], X(901:1200,:), Group(1:300), 'Preprocessing', 1);
rj5 = omedaPca(ascao.factors{2}.matrix, [1], ascao.factors{2}.matrix, Group, 'Preprocessing', 1);
...
%% ============================================================
% LOCAL FUNCTION:
% Score 1 / Score 2 individual exports (NO legend)
%% ============================================================
...
function plotTwoColumnLoadingHeatmap(L_plot, taxaNames, colNames, outBase, exportResolution)
    ...
    cellW = 0.52;   % keep width fixed
    cellH = 0.32;   % taller rows for clearer taxa labels
    ...
    for r = 0.5:1:(nRows + 0.5)
        plot(axLoad, [0.5 nCols + 0.5], [r r], '-', ...
            'Color', [0.85 0.85 0.85], ...
            'LineWidth', 0.5);
    end

    for c = 0.5:1:(nCols + 0.5)
        plot(axLoad, [c c], [0.5 nRows + 0.5], '-', ...
            'Color', [0.85 0.85 0.85], ...
            'LineWidth', 0.5);
    end
    ...
    if showValues
        for i = 1:nRows
            for j = 1:nCols
                ...
                val = L_plot(i,j);
                valRounded = round(val, nDec);

                if abs(valRounded) < 5e-4
                    labelStr = '0';
                else
                    labelStr = sprintf('%.2f', valRounded);
                end
...
function plotOmedaHeatmapCompact(O, methodNames, outBase, exportResolution)
    ...
    cellW = 0.48;   % keep width fixed
    cellH = 0.32;   % taller rows
    ...
    for r = 0.5:1:(nRows + 0.5)
        plot(axOmeda, [0.5 nCols + 0.5], [r r], '-', ...
            'Color', [0.85 0.85 0.85], ...
            'LineWidth', 0.5);
    end

    for c = 0.5:1:(nCols + 0.5)
        plot(axOmeda, [c c], [0.5 nRows + 0.5], '-', ...
            'Color', [0.85 0.85 0.85], ...
            'LineWidth', 0.5);
    end
...
function createFinal_CASCA_oMEDA_Panel( ...
    ...
    L_plot, taxaNames, O, omedaMethodNames, outBase, analysisLevel)
    ...
    %% ---------------------------------------------------------
    % Layout:
    % Genus: original tall heatmaps.
    % Phylum: half-height heatmaps and reduced gap between rows.
    %% ---------------------------------------------------------

    if lower(string(analysisLevel)) == "phylum"

        % Phylum has only few taxa.
        % Heatmaps are half-height.
        % Top row moved downward to remove excessive vertical white space.
        topY = 0.725;
        topH = 0.110;
        ...
        % Genus has many taxa.
        % Keep original tall heatmap layout.
        topY = 0.865;
        topH = 0.110;
...
function addPanelLabel(fig, pos, txt)
    ...
    % Place label just above the panel, but keep it inside the figure.
    % This avoids both clipping and overlap with the panel itself.

    labelW = 0.040;
    labelH = 0.022;

    % Horizontal position: slightly left of panel
    x = pos(1) - 0.012;

    % Vertical position: slightly ABOVE panel
    y = pos(2) + pos(4) + 0.004;

    % Keep inside figure boundary
    if x < 0.002
        x = 0.002;
    end
...
%% ============================================================
% PANEL A / B: 1D SCORE PLOT (NO LEGEND CODE AT ALL)
%% ============================================================
...
function plotPanelLoadingHeatmap(axMain, axTaxa, axCB, L_plot, taxaNames, colNames)
    ...
    for r = 0.5:1:(nRows + 0.5)
        plot(axMain, [0.5 nCols + 0.5], [r r], '-', ...
            'Color', [0.88 0.88 0.88], ...
            'LineWidth', 0.25);
    end

    for c = 0.5:1:(nCols + 0.5)
        plot(axMain, [c c], [0.5 nRows + 0.5], '-', ...
            'Color', [0.88 0.88 0.88], ...
            'LineWidth', 0.25);
    end
